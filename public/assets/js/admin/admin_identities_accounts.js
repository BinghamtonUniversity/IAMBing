ajax.get('/api/identities/'+id+'/accounts',function(data) {
    ...
    schema:[
        ...
        {type: "select","label": "Status","name": "status","options": [{"label": "Active","value": "active"},{"label": "Deleted","value": "deleted"}]}
    ], data: data
    ...
    //     }).modal().set(grid_event.model.attributes)
        ...
        if (grid_event.model.attributes.status === 'active') {
            toastr.error("You cannot restore an account which is active!");
            return;
        }
